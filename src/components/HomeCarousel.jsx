...
import '../assets/css/style.css'
    ...
    const HomeCarousel = () => {
        ...
        const nextSlide = () => {
        setCurrentSlide((prev) => (prev + 1) % cardsList.length);
        };
    
        const prevSlide = () => {
        setCurrentSlide((prev) => (prev - 1 + cardsList.length) % cardsList.length);
        };
        ...
        <div className="relative mx-auto mt-10 overflow-hidden">
            ...
            {Array.isArray(cardsList)&&cardsList.slice().map((cards) => (
                <div className="flex-shrink-0 w-full" key={cards.CardsID}>
                    <div className="relative flex justify-center w-full h-[500px] md:h-[700px] lg:h-[800px] xl:h-[900px]">
                        <div className="relative flex items-center justify-center w-full h-[500px] p-4 md:h-[700px] xl:h-[750px]">
                            ...
                            <div className="relative w-full max-w-[90%] z-10 mx-auto">
                                <img className="object-fill pb-20 pl-16 md:pl-48 xl:pl-96" src={cards.CardImage} />
                            </div>
                        </div>
                        {/* 文卡片 */}
                        <div className="absolute left-1/4 w-2/5 bg-[#ffffff] shadow-lg transform -translate-x-1/3 translate-y-[40%] z-20 border border-solid border-[#8F755A] mx-auto  md:w-1/3 m-3 ">
                            ...
                            <p className="p-2 mb-6 overflow-y-auto font-normal text-left font-bodyFont text-p-3 md:text-p-2 lg:text-p-1 text-indent-1 lg:p-5 xl:px-20 xl:pb-20">{cards.Paragraph}</p>
