...
import './calendar.css'; // 自定義樣式

const Calendarr = ({selectedDate, setSelectedDate}) => {
        ...
        <Card>
            <CardBody>
                <div className="flex space-x-6 text-p-2 h-52 scale-95">
                    ...
                    <Calendar
                        className="calendar text-brown 
                        hover:bg-lightbrown hover:scale-105 hover:text-lightyellow hover:font-bold
                        transition-all"
                        onClickDay={handleDateClick} // 點擊某天時觸發事件
                        onChange={setValue}
                        value={value}
                    ...
                    <Calendar className="calendar-hidden text-lightbrown "
                        value={new Date(activeStartDate.getFullYear(), activeStartDate.getMonth() + 1)} // 顯示下個月
                        onActiveStartDateChange={handleDateChange} // 同步月份
                        tileContent={tileContent}
                        tileDisabled={tileDisabled}
                    />
