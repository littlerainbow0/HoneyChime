        ...
        <div style={{ overflowY: 'auto', height: '100%' }}>
          <table className="text-center bg-gray-50 rounded-3xl" style={{ width: '100%', tableLayout: 'fixed' }}>
              ...
              <tr>
                {columns.map((column) => (
                  <th key={column} className='py-2'>
                    <p className="m-1 text-lightbrown font-bodyFont text-p-2">{columnNames[column] || column}</p>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {currentRows.map((item, index) => (
                <React.Fragment key={item.ScheduleID}>
                  ...
                  hover:bg-gray-200 hover:translate-y-1 ${index % 2 === 0 ? "bg-lightyellow" : ""}`}>
                    {columns.map((column) => (
                      <td key={column}>
                        <p className="mb-1 text-dark font-bodyFont text-p-3">{item[column]}</p>
                      </td>
                    ))}
                    ...
                    <td>
                      <Button className='transition-all border-transparent rounded-full text-brown bolder hover:text-lightyellow hover:shadow-md hover:bg-lightbrown' onClick={() => clickShowModalToEdit(item)}>
                        <RiEdit2Fill />
                      </Button>
                    </td>
                    ) : (<td className='hidden' />)}
                    ...
                    <td>
                      <Button className='m-2 transition-all border-transparent rounded-full text-brown bolder hover:text-darkbrown hover:shadow-md'
                            ...
                            <thead>
                              <tr>
                                {detailColumns.map((column) => (
                                  <th key={column}>
                                    <p className="py-1 m-1 mx-10 text-gray-500 bg-gray-300 rounded-full font-bodyFont text-p-3">
                                  ...
                                  <tr key={subIndex}>
                                    {detailColumns.map((column) => (
                                      <td key={column} className='rounded-full'>
                                        <p className="mb-1 text-gray-500 font-bodyFont text-p-3">{subItem[column]}</p>
                                      </td>
                                    ))}
                                  </tr>
                                ))}
              ...
              <tr>
                <td>
                  <span className='text-gray-100 bg-gray-100'>123</span>
                </td>
              </tr>
            </tbody>
          </table>
